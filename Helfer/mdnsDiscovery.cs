...
namespace isci
{
    public class mdnsDiscovery
    {
        ...
        public Dictionary<string, List<Entdeckung>> Anwendungen = new Dictionary<string, List<Entdeckung>>();
            ...
            mdns.AnswerReceived += (s, e) =>
            {
                if (e.Message.Answers.Count == 2)
                {
                    if (e.Message.Answers[0].CanonicalName.Contains(".isci."))
                    {
                        ...
                        if(!Anwendungen.ContainsKey(neueEntdeckung.Anwendung)){
                            // Wenn Anwendung des gefundenen Modulls noch nie gefunden wurde, dann erstelle sie
                            Anwendungen.Add(neueEntdeckung.Anwendung, new List<Entdeckung>());
                        }
                        if(!Anwendungen[neueEntdeckung.Anwendung].Contains(neueEntdeckung)){
                            // Wenn das gefundene Modull neu ist, füge es der Liste hinzu
                            Anwendungen[neueEntdeckung.Anwendung].Add(neueEntdeckung);
                        }
                    }
                }
